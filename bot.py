...
def main_menu():
    ...
    kb.add("⭐ Stars ishlash")
    kb.add("👤 Profil", "🎁 Gift olish")
    kb.add("📣 To‘lov kanali", "🛠 Support")
    return kb
...
async def start(msg: types.Message):
    ...
    await msg.answer("🔥 Stars botga xush kelibsiz!", reply_markup=main_menu())
...
async def gift(msg: types.Message):
    ...
    kb = InlineKeyboardMarkup().add(
        InlineKeyboardButton("✅ Tasdiqlash", callback_data=f"ok_{msg.from_user.id}"),
        InlineKeyboardButton("❌ Rad etish", callback_data=f"no_{msg.from_user.id}")
    )
...
if call.data.startswith("ok_"):
        ...
        db.commit()
        await bot.send_message(uid, "❌ Ariza rad etildi")
